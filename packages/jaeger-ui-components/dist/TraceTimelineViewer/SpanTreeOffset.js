...
export default class SpanTreeOffset extends React.PureComponent {
    constructor(props) {
        super(props);
        /**
    ...
    render() {
        ...
        const icon = showChildrenIcon && hasChildren && (childrenVisible ? React.createElement(IoIosArrowDown, null) : React.createElement(IoChevronRight, null));
        ...
        return (React.createElement("span", Object.assign({ className: cx(styles.SpanTreeOffset, { [styles.SpanTreeOffsetParent]: hasChildren }) }, wrapperProps),
            this.ancestorIds.map(ancestorId => (React.createElement("span", { key: ancestorId, className: cx(styles.indentGuide, {
                    [styles.indentGuideActive]: this.props.hoverIndentGuideIds.has(ancestorId),
                }), "data-ancestor-id": ancestorId, "data-test-id": "SpanTreeOffset--indentGuide", onMouseEnter: event => this.handleMouseEnter(event, ancestorId), onMouseLeave: event => this.handleMouseLeave(event, ancestorId) }))),
            icon && (React.createElement("span", { className: styles.iconWrapper, onMouseEnter: event => this.handleMouseEnter(event, spanID), onMouseLeave: event => this.handleMouseLeave(event, spanID), "data-test-id": "icon-wrapper" }, icon))));
    }
}
